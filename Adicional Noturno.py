...
for worksheet_hours in workbook_hours.worksheets[0:1]:
    ...
    for registration_cell, add_cell, add_min_cell, unity_cell in zip (worksheet_hours['A:A'],worksheet_hours['C:C'],worksheet_hours["E:E"], worksheet_hours["I:I"]): 
        if registration_cell.value is not None:
            if add_cell.value in ["SIM","sim","Sim"]:
                if add_min_cell.value !=0 and add_min_cell.value is not None:
                    ...
                    list_sorted=sorted(list_addition, key=lambda list:list[4])
                    itens_list +=1
                    print (*list_sorted, sep='\n')

while itens_list > count:
    worksheet_base.cell(row=count+5,column=1).value = list_sorted[0+count][0]
    worksheet_base.cell(row=count+5,column=2).value = list_sorted[0+count][1]
    worksheet_base.cell(row=count+5,column=3).value = list_sorted[0+count][2]
    worksheet_base.cell(row=count+5,column=7).value = list_sorted[0+count][3]
    worksheet_base.cell(row=count+5,column=11).value = list_sorted[0+count][4]
    workbook_base.save(filepath + "\\Gerados\\Tabela Adicional Noturno Professores" + "(" + str_current_datetime + ")" + ".xlsx")
    count +=1
